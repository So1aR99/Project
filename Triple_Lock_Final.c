...
int adc_max_flag = 0;                                           // ADC(가변저항) 최대값 도달 상태
int password_stage = 0;                                         // 비밀번호 입력 단계
int timeout = 0;                                                // 타임아웃 여부 ( 1:발생, 0:발생x )
int timer_start = 0;                                            // 타이머 시작 여부
unsigned int prev_adc = 0;

// 시리얼 통신 관련 변수
char Final_pass = 'U';                                          // 3번째 잠금 비밀번호
...
void LCD_Init(void)
{
    LCD_Comm(0x38); _delay_ms(2);
    LCD_Comm(0x0E); _delay_ms(2);
    LCD_Comm(0x06); _delay_ms(2);
    LCD_Clear();
}
...
int USART_Available(void)
{
    return (UCSR0A & (1 << RXC0));								// 수신된 데이터가 있는지 확인
}
    ...
    while (1)
    {
        ...
        if (adc_value >= 1020)
        {
            if (!adc_max_flag)                                  // 처음 최대값에 도달하면
            {
                ...
                adc_max_flag = 1;                               // ADC 최대값 상태 설정
                password_stage = 1;                             // 비밀번호 입력 단계 설정
            ...
            if (timeout)                                        // 타임아웃이 발생되면
            {
                ...
                for (int i = 0; i < 20; i++)                    // 1,2번 LED가 20번
                {
                    PORTA |= 0x01;                              // 1번 LED ON
                    _delay_ms(50);
            ...
            if (password_stage == 1)                            // 비밀번호 입력 단계일 때
            {
                ...
                if (key != 0)                                   // 키가 눌리면
                {
                    if (input_index < PASSWORD_LENGTH && ((key >= '0' && key <= '9') || key == '*' || key == '#'))
                    {
                        ...
                        LCD_pos(1, input_index);                // 두번째줄, 입력위치로 커서이동
                    ...
                    if (input_index == PASSWORD_LENGTH)        // 비밀번호 입력 완료 시
                    {
                        ...
                        else                                    // 비밀번호가 틀리면
                        {
                            for (int i = 0; i < 5; i++)
                            {
                                LCD_pos(0, 0);
                                LCD_STR("Fail!!!!");
                                PORTA ^= 0x02;                  // 2번 LED 300ms로 5번 깜빡임
                                _delay_ms(300);
                                LCD_Clear();
                                PORTA ^= 0x02;
                                _delay_ms(300);
        ...
        {
            if (adc_max_flag)                                   // 이전에 최대값이였으면 초기화
            {
                _delay_ms(500);
                LCD_Clear();
                LCD_pos(0, 0);
        ...
        {
            unsigned char received_char = USART0_rx();

            USART0_tx(received_char);                           // 에코

            if (received_char == Final_pass)                    // 문자 비교
            {
                ...
                if (third_lock_open)                            // 3단계 잠금 해제되면
                {
                    ...
                    while (1) 
                    {
                        ...
                        if (current_adc < 1020)                 // ADC값 변화하면 종료
                        {										
                            break;
                        }
                        StepMotor();
